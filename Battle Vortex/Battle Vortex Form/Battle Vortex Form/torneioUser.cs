...
{
    public partial class torneioUser : Form
    {
        public torneioUser()
        {
            InitializeComponent();
        ...
        private void CarregarDados()
        {
            using (MySqlConnection conexao = new MySqlConnection("SERVER=127.0.0.1; DATABASE=eventosbv; UID=root; PASSWORD=;"))
            {
                ...
                MySqlCommand consultaColunas = new MySqlCommand("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'eventosbv' AND TABLE_NAME = 'torneios_com_premios';", conexao);
                ...
                string query = "SELECT * FROM `torneios_com_premios`";
                ...
                if (dataGridView1.Columns.Contains("premio_id"))
                {
                    dataGridView1.Columns["premio_id"].Visible = false;
                }

                if (dataGridView1.Columns.Contains("torneio_id"))
                {
                    dataGridView1.Columns["torneio_id"].Visible = false;
                }
        ...
        {
            torneioInscrever torneioInscrever = new torneioInscrever();
            torneioInscrever.ShowDialog();
            CarregarDados();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection conexao = new MySqlConnection("SERVER=127.0.0.1; DATABASE=eventosbv; UID=root; PASSWORD=;"))
            {
                try
                {
                    ...
                    // Consulta de filtragem usando a view torneios_com_premios
                    string query = $"SELECT * FROM torneios_com_premios WHERE `{campo}` LIKE @valorCampo";
